...
class EffectFooter{
	...
	static function PageLoading(){
		// 插件所在位置的路径信息
		//$PluginPath = Helper::options()->pluginUrl.'/WenYu/';
		$PluginPath='https://zpblogs.gitee.io/typecho_wenyu/';

		// 获取配置信息
		$type = Typecho_Widget::widget('Widget_Options')->plugin('WenYu')->MouseStyle;
		echo'<script type="text/javascript" src="https://www.zpblogs.cn/zp/cj/public.js"></script>';
	}
	...
	static function MouseStyle(){
		...
		if ($type == 0) {
			...
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/mouse_snow.min.js"></script>';
		}
	...
	static function MouseStyle2(){
		...
		// 获取配置信息
		$type = Typecho_Widget::widget('Widget_Options')->plugin('WenYu')->MouseStyle2;
	
		
		if ($type == 0) {

		}else if($type==1){// 爱心
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/love.js"></script>';
		}else if($type==2){// 粒子
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/particle.js"></script>';
		}else if($type==3){// 气泡1
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/bubble.js"></script>';
		}else if($type==4){// 气泡2
			echo'<div id="clickCanvas" style=" position:fixed;left:0;top:0;z-index:999999999;pointer-events:none;"></div>';
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/cursor.js"></script>';
		}else if($type==5){// 随机文字
			echo '
			<script type="text/javascript">
				...
				jQuery(document).ready(function ($) {
					$("body").click(function (e) {
						...
						// var a = new Array("Hello基佬", "你再点", "你继续点", "你有本事接着点", "有种你还点", "快别他妈点了", "好好看文章不行吗？", "哎哟卧槽不和你玩了！");
	...
	static function PrettifyStyle(){
		...
		if ($type == 0) {
			...
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/particles.js"></script>';
		}else if($type==2){// 花瓣雨
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/sakura.js"></script>';
		}else if($type==3){// 彩带
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/ribbon.js"></script>';
		}else if($type==4){// 气泡
			echo'<div id="box" style="position: fixed;top: 0px;width:100%;height:auto;z-index: -1;"></div>';
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/qipao/particles.js"></script>';
			echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/qipao/background.js"></script>';
		}
	...
	static function Smallfunction(){
		...
		if(!empty($type)){
			...
			//打字特效
			if(in_array("CommentTyping",$type)){
				echo '<script type="text/javascript" src="' . $PluginPath . 'assets/js/commentTyping.js"></script>';
			}
			//崩溃欺骗
			if(in_array("bkqp",$type)){
				echo '
				<script>
					//崩溃欺骗
			...
			//猫咪打字动画
			if(in_array("mmdzdh",$type)){
				echo '
					<link rel="stylesheet" href="' . $PluginPath . 'assets/css/cat.css"></script>
	...
	static function htStyle(){
		...
		if(!empty($type)){
			...
			if(in_array("dlmh",$type)){
				...
				<script>
					$(function(){
						$(".i-logo").attr("href","javascript:;");
						$(".more-link").append("<a href=\"https://www.zpblogs.cn\" target=\"_blank\">闻语博客</a>")
					})
				</script>
				';
			}
